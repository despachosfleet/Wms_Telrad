...
const PickingListaView = {
  ...
  async afterRender() {
    ...
    cont.innerHTML = '<div style="display:flex; flex-direction:column; gap:8px;">' +
      pendientes.map(d => `
        <div class="stock-card" data-id="${d.id}">
          <div class="stock-card-top">
            <p class="stock-sku">${escapeHtml(d.gr || 'Sin GR')}</p>
            <span class="pill pill-warning">Pendiente</span>
          </div>
          <div class="stock-meta">
            <span><span class="meta-label">Cliente:</span> <b>${escapeHtml(d.cliente || '-')}</b></span>
            <span><span class="meta-label">Destino:</span> <b>${escapeHtml(d.destino || '-')}</b></span>
          </div>
          <div class="stock-meta" style="margin-top:4px;">
            <span><span class="meta-label">Fecha:</span> <b>${escapeHtml(d.fecha || '-')}</b></span>
          </div>
        </div>
      `).join('') +
      '</div>';
