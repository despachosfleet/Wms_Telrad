// ============================================================
// CONSULTA DE STOCK — filtros sticky, fila expandible
// ============================================================
const ConsultaView = {
    ...
    return `
      <div class="sticky-filters">
        ...
        <div style="margin-bottom:10px;">
          <div style="font-size:var(--font-xs); font-weight:700; text-transform:uppercase; letter-spacing:.5px; color:var(--text-secondary); margin-bottom:6px;">Estado</div>
        ...
        <div class="btn-row">
          <button class="btn-primary" id="btn-buscar-stock">Buscar</button>
  ...
  _renderTabla(cont) {
    ...
    cont.innerHTML = `
      <p style="font-size:11px; color:var(--text-tertiary); margin-bottom:6px;">${this._resultados.length} resultado${this._resultados.length!==1?'s':''}</p>
      <div class="table-wrap">
        <table class="data-table" id="tabla-stock-result">
          <thead><tr>
            ${th('sku','SKU')}
  ...
  _renderFilas() {
    return this._resultados.map((r,i) => {
      const expRow = this._expandidaFila === i;
      ...
      return `
        <tr data-fila="${i}" style="cursor:pointer;">
          ...
          <td style="font-weight:700; color:${Number(r.cantidad)<=0?'var(--danger-text)':'var(--text)'};">${formatNum(r.cantidad)} ${escapeHtml(r.unidad_medida||'')}</td>
  ...
  _bindFilas() {
    document.querySelectorAll('[data-fila]').forEach(tr => {
      tr.addEventListener('click', () => {
        ...
        if (tbody) tbody.innerHTML = this._renderFilas();
        this._bindFilas();
      });
    });
  }
