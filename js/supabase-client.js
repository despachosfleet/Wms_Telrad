...
async function buscarStock({ texto = '', filtro = 'TODOS', limit = 50 } = {}) {
  ...
  if (texto) {
    const t = texto.trim();
    query = query.or(
      `sku.ilike.%${t}%,serie.ilike.%${t}%,paleta_pedido.ilike.%${t}%,ubicacion_fisica.ilike.%${t}%,descripcion.ilike.%${t}%`
    );
  }
...
async function confirmarPicking(itemDespachoId, stockId, cantidadPickeada, usuario = '') {
  ...
  const { data: stockRow, error: errGet } = await supabase
    .from('stock')
    .select('*')
    .eq('id', stockId)
    .single();
...
async function moverPaletaCompleta(paletaPedido, nuevaUbicacion, usuario = '') {
  const { data: items, error: errGet } = await supabase
    .from('stock')
